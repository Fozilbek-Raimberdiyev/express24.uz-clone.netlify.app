<template>
  <div>
    <swiper :options="options" class="tab flex flex-wrap  items-center mt-10">
        <swiper-slide class="tab-btn mb-3 mr-3 rounded-xl" :class="{active : currentIndex==index}" v-for="(category,index) in categories" :key="index"><span @click="changeCurrentId(category)">{{category.name}}</span></swiper-slide>
    </swiper>
    ...
    <button v-if="allPlaces?.length" class="btn rounded" @click="seeMore">Показать еще</button>
  </div>
</template>
...
export default {
    ...
    watch : {
        ...
        currentId(newVal,oldValue) {
            if(this.currentPlace == "market") {
                if(newVal!=oldValue) {
                    this.getMarkets()
                }
            } else {
    ...
    methods: {
        ...
        async getCategoriesMarkets() {
            let res = await fetch("https://express24.uz/rest/v2/catalog/categories/?root_category_id=2");
            let resJson  = await res.json();
            this.categories = resJson.list
        },
        rate(item) {
        ...
        ...mapActions([
            "getRestaurants", "getMarkets", "getFilteredRestaurants", "getFilteredMarkets", "getFilteredPlaces"
        ]),
        ...mapMutations([
            "changeCurrentId"
        ])
